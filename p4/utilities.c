...
/*
 * @fn char* CopyString(char* p)
 * @brief Allocate and copy string argument.
 *        Responsibility of caller to free memory.
 * @param char* pString [in] String to copy
 * @return copy of argument string
 */
char* CopyString(char* pString)
{
	size_t szString = strlen(pString);
	char* pCopy = Malloc(szString + 1);
...
{
	int out;
	out = pthread_cond_broadcast(cond);
	if (out != 0)
	{
		printf("utilities.c:pthread_cond_broadcast:"
